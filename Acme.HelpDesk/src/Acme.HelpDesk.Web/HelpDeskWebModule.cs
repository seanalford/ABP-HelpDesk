...
using Acme.HelpDesk.MultiTenancy;
...
namespace Acme.HelpDesk.Web
{
    ...
    public class HelpDeskWebModule : AbpModule
    {
        ...
        private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
        {
            ...
            {
                options.FileSets.AddEmbedded<HelpDeskWebModule>("Acme.HelpDesk.Web");

                if (hostingEnvironment.IsDevelopment())
                {
                    options.FileSets.ReplaceEmbeddedByPhysical<HelpDeskDomainSharedModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}Acme.HelpDesk.Domain.Shared", Path.DirectorySeparatorChar)));
                    options.FileSets.ReplaceEmbeddedByPhysical<HelpDeskDomainModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}Acme.HelpDesk.Domain", Path.DirectorySeparatorChar)));
                    options.FileSets.ReplaceEmbeddedByPhysical<HelpDeskApplicationContractsModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}Acme.HelpDesk.Application.Contracts", Path.DirectorySeparatorChar)));
                    options.FileSets.ReplaceEmbeddedByPhysical<HelpDeskApplicationModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}Acme.HelpDesk.Application", Path.DirectorySeparatorChar)));
                    options.FileSets.ReplaceEmbeddedByPhysical<HelpDeskHttpApiModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}Acme.HelpDesk.HttpApi", Path.DirectorySeparatorChar)));
